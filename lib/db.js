...
async function bulk(index, body, id = "", sq = {}) {

    const Transpose = (x) => {
        const keys = Object.keys(x);
        return x[keys[0]].map((_, index) => {
    ...
    const bulkRequestBody = Transpose(body).flatMap((ele) => [
        { index: { _index: index } },
        ele
    ]);

    const a = await fetch(`http://localhost:9200/_bulk`, {
        "method": "POST",
        "headers": {
            "Content-Type": "application/json"
        },
        "body": bulkRequestBody.map(JSON.stringify).join('\n') + '\n'
    })
    const out = (await a.json());
    if ("error" in out) throw new Error(JSON.stringify(out))
    return out
...
export async function addBWList(name, value) {
    ...
    const out = await bulk(name.toLowerCase(), { phoneNum: value }).then(out => {
