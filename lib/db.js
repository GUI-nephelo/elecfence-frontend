...
// -------------------------------------------------------------------------------------------------
export async function getData({ page, pageSize, searchString, whichCol }) {
    var searchBody = {};
    if (searchString) {
...
export async function addBWList(name, value) {
    ...
    await put(name.toLowerCase(), { phoneNum: value })
    await pipelineCtrl(name)
}

export async function deleteBWList(name, value) {
    const out = await del(name.toLowerCase(), {
        query: { match: { phoneNum: value } }
    })
    await pipelineCtrl(name)
}
// -------------------------------------------------------------------------------------------------

const setPipeline = async (name, list) => {
    var ele;
    switch (name[0]) {
        case 'b':
            ele = { "script": { "source": `ctx.inBlackList=(${JSON.stringify(list)}.contains(ctx.phoneNum));` } }
            break;
        case 'w':
            ele = { "drop": { "if": `${JSON.stringify(list)}.contains(ctx.phoneNum)` } }
            break;
    }
    const raw = await fetch(`http://localhost:9200/_ingest/pipeline/${name.toLowerCase()}-filter-pipeline`, {
        "method": "PUT",
        "headers": { "Content-Type": "application/json" },
        "body": JSON.stringify({ processors: [ele] })
    });
    const out = await raw.json()
    return out
}
...
export async function pipelineCtrl(name) {
    ...
    console.log("setPipeline", name, list)
    const out = await setPipeline(name, list)
    console.log(out)
}
